...
- **Script/payload:** processo real descrito em [[../../Skills/SKILL_SYNC_QASE|SKILL_SYNC_QASE]] — pasta `sogov-automation-test/scripts/qase-sync-<contexto>/` (`sync.js` + `corrections.json` + `README.md`), copiada da versão mais recente já usada.
...
**Nunca inventar rótulo de enum.** Rótulos confirmados até 24/09/2026: `severity: normal` (4), `type: acceptance` (7), `automation: is-not-automated` (0). Não existe `type` de "regressão" confirmado neste projeto — CTs de regressão usam `acceptance` mesmo, sinalizados pelo prefixo `[REGRESSÃO]` no título.

`priority` e `behavior` ficam de fora do payload por padrão — preencher manualmente na Qase depois, se fizer sentido.

Tags da nota: manter somente `qa` e `qase`. Tags enviadas ao Qase: ID da demanda + módulo (ex. `SGV-XXXX`, `tramitação`) — não criar uma tag por CT.
...
- **Descrição:** <resumo do objetivo + critério coberto>
- **Pré-condições:** <estado necessário antes do teste>
- **Passo 1 — Ação:** <ação>
  **Resultado esperado:** <resultado>
- **Pós-condição:** <estado após o teste, se agregar algo além do resultado>
