...
| TR 1.24-1.25, Suíte 4 (`identity-lifecycle.api.cy.js`) — 02/09/2026 (reaproveitamento de dados) | Os 5 agentes da suíte usavam `generateCPF()` (novo a cada rodada) mesmo todos já devolvendo o estado pra Ativo no fim do fluxo — 10 registros novos por rodada completa (Suíte 3+4), sem necessidade real. Cruzamento dos cenários mostrou que só o CT-035 (Suspenso) não revertia pro estado Ativo no final, o que quebraria o reaproveitamento dos outros 4 (busca por nome do `getPublicAgentOrCreate` na próxima rodada). | `fixedCpf` opcional adicionado em `cy.createIsolatedTestAgent`; os 5 CPFs da Suíte 4 fixados; CT-035 passou a reverter pro Ativo no final. Suíte 3 (lockout) mantida com CPF novo por rodada — decisão consciente, sem mutation de desbloqueio ainda. **Verificação em execução real pendente** (não rodado ainda) |
