...
5. **Destilar → card:** criar o card em `02 Demandas/<ambiente>/` a partir da seção Destilado ([[SKILL_BUGS]] pra bug; [[../Templates/Demanda.md|Demanda.md]] pra melhoria/funcionalidade) — só o problema, sem análise/suposição. **O link pra mesa arquivada vai logo no topo do card** (dentro do callout `[!info] Informações`, ou logo após o H1 em cards sem esse callout — nunca só em Observações, que fica soterrado no fim de cards longos e é fácil de perder quando o card muda de pasta). Daily: `📝 SGV-XXXX - <Tipo> refinado(a)` (copy oficial) + **regra de links** (toda menção ao SGV vira wikilink pro card).

   **Resumo em linguagem simples (quando fizer sentido):** demanda com múltiplos RFs/CAs ou jargão técnico denso (schema, nomenclatura de dev) ganha também um `Tasks/<SGV>/<SGV> - Resumo.md`, ao lado da mesa arquivada — o que é, qual problema resolve, o que passa a funcionar quando pronto, sem CT nem critério técnico (esses ficam só no card). É a porta de entrada pra quem quer entender rápido antes de ir pro detalhe técnico. Não é proporcional pra bug simples de 1-2 critérios — aí o card já é direto o bastante. Precedente: SGV-11184 (03/09/2026).

   **Ordem de leitura e nome de arquivo em `Tasks/<SGV>/`:** quando a pasta tem mais de um arquivo (Resumo + mesa, ou mesa + Plano de Automação), prefixar o nome com um número que force a ordem certa na listagem do Obsidian (que ordena por nome, e "Refinamento" vem antes de "Resumo" e "Plano" alfabeticamente — o oposto da ordem de leitura). Ordem: `1 - <SGV> - Resumo.md` → `2 - <SGV> - Refinamento <título>.md` → `3 - <SGV> - Plano de Automação.md` (quando existir). Num épico multi-parte, o índice em `Tasks/<épico>/` leva `0 - ` pra ordenar antes das subpastas das partes. O prefixo é só o começo do nome — busca por `SGV-XXXX` continua achando normalmente. H1 de cada tipo segue o mesmo padrão `Tipo: Título` que `Demanda:`/`Refinamento:` já usam: `# Resumo: <Título>`, `# Índice: <Título>`, `# Plano de Automação: <Título>`. Ordem de leitura, do mais geral ao mais técnico: Índice (se houver) → Resumo (se houver) → card (critérios/CTs) → mesa de refinamento → Plano de Automação.
