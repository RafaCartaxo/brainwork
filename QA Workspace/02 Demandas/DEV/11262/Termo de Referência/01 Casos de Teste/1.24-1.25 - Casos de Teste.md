...
Casos de teste no template padrão do vault ([[Sistema/Skills/SKILL_CASOS_DE_TESTE|SKILL_CASOS_DE_TESTE.md]]), a partir do Termo de Referência 1.24–1.25. **Fonte única** deste Termo — ver revisão de 31/08 abaixo. Fontes:

> [!warning] Pendência (24/09/2026): migrar pro padrão de pacote
> Este arquivo ainda usa o formato de nota única (`#### **CT-NNN**`); o template solto `Casos de teste.md` que ele referenciava foi removido — o formato segue descrito em `SKILL_CASOS_DE_TESTE`. Este Termo vai entrar em conformidade com o padrão de pacote ([[Sistema/Templates/Pacote/00 README|Sistema/Templates/Pacote/]]) como as demais demandas — ainda não migrado.
- Planilha original (intocada): [[1.24-1.25: Execução|1.24-1.25: Execução.md]] — desatualizada, mantida só para histórico.
- Organização anterior por suites/shared steps nativos da Qase: [[1.24-1.25 - Casos organizados para Qase|1.24-1.25 - Casos organizados para Qase.md]] — **arquivada em 31/08**, conteúdo absorvido por este arquivo.
