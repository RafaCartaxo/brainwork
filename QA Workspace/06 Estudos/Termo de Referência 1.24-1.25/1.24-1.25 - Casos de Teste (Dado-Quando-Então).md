# 1.24–1.25 (Autenticação e ciclo de vida do usuário) — Casos de Teste

Casos de teste no template padrão do vault ([[Sistema/Skills/SKILL_CASOS_DE_TESTE|SKILL_CASOS_DE_TESTE.md]] / [[Sistema/Templates/Casos de teste|Casos de teste.md]]), a partir do Termo de Referência 1.24–1.25. Fontes:
- Planilha original (intocada): [[QA Workspace/06 Estudos/Termo de Referência 1.24-1.25/1.24-1.25: Execução|1.24-1.25: Execução.md]]
- Organização por suites/shared steps nativos da Qase, com prioridade e rastreabilidade completa por item de requisito: [[QA Workspace/06 Estudos/Termo de Referência 1.24-1.25/1.24-1.25 - Casos organizados para Qase|1.24-1.25 - Casos organizados para Qase.md]]

Numeração `CT-001`–`CT-039`, única e contígua ao longo das 5 suites (tratando este Termo de Referência como a demanda/escopo de numeração). Um único item (TC-05 da planilha original) foi removido por não corresponder a nenhum cenário real — ver seção **G** ao final.

> [!info] Revisão de 18/08 — contexto de arquitetura real + pós-condições
> Depois da primeira versão deste arquivo, o Rafael deu contexto de arquitetura que o Termo de Referência original não tinha, e que resolveu **todos os 6 Gaps** da versão anterior. Resumo do que mudou:
> - **Cidadão CPF (pessoa física) e Cidadão CNPJ (empresa) usam a mesma tela de login**, com um **campo único** (sem seletor PF/PJ) — não são duas telas separadas.
> - **Servidor tem tela própria**, separada da tela do cidadão.
> - **Todo servidor também é cidadão**: logar na tela do cidadão com o próprio CPF funciona, mas autentica no **contexto de cidadão** (CT-008).
> - **Empresa autentica exclusivamente por CNPJ** — não existe login por "responsável legal" via CPF (por isso o antigo CT-005 foi removido, seção G).
> - **Mecanismo de desbloqueio** (CT-016/CT-017): link por e-mail (autosserviço) ou desbloqueio manual por um servidor.
> - **Mudança de status é aplicada imediatamente** na sessão em curso, para qualquer status (CT-025, CT-034).
> - **Licença/Férias hoje não têm acesso de leitura** — o Termo não exige que o "mínimo" inclua visibilidade, então isso não é uma inconformidade (correção nos antigos CT-024/CT-030, que diziam o contrário).
> - **"Suspenso" se comporta como "Inativo"** (CT-036) — a ausência do termo no item 1.25.3 era só ambiguidade de redação.
> - **Múltiplas sessões simultâneas são permitidas** (CT-039).
> - **Pós-condições** adicionadas em todos os 39 casos, explicitamente (inclusive onde não há efeito persistente).

---

## Legenda — Shared Steps

Cada `Quando`/`E` abaixo já traz a ação por extenso; a tag `[SS-0N]` ao lado é só rastreabilidade — indica que aquele passo é uma instância de uma sequência reutilizada em outros casos. Um shared step só existe aqui se for de fato reusado em 2+ casos.

| ID | Ação completa |
|---|---|
| **SS-01a** | Acessar a tela do **servidor** (exclusiva, CPF apenas), informar o CPF cadastrado e a senha correta, e confirmar |
| **SS-01b** | Acessar a tela do **cidadão** — campo único de identificação, compartilhado entre CPF (pessoa física) e CNPJ (empresa) — informar o identificador correto e a senha correta, e confirmar |
| **SS-02** | Acessar a tela de login, informar um identificador válido e uma senha incorreta, e tentar autenticar (1 tentativa) |
| **SS-03** | Repetir o SS-02 **N** vezes seguidas para o mesmo usuário (N varia por caso) |
| **SS-04** | Com o servidor autenticado em status de afastamento legal (Licença ou Férias): **(a)** confirmar que o login foi permitido; **(b)** observar o aviso de status na tela inicial; **(c)** tentar uma ação de escrita/aprovação — deve ser bloqueada; **(d)** tentar uma ação de leitura/consulta — **hoje sem nenhum acesso, nem de leitura** (o "subconjunto mínimo de funcionalidades não transacionais" do Termo não obriga a incluir visibilidade) |
| **SS-05** | Aguardar (ou simular) a data de término do período de Licença/Férias do servidor, e autenticar novamente |

---

## Suite 1 — Tipos de acesso (1.24)

### **CT-001 Servidor Público autentica com sucesso usando CPF e senha corretos**

**Dado** que o servidor está cadastrado, vinculado a um setor e com status "Ativo"
**Quando** ele acessa a tela de login do servidor, informa o CPF cadastrado e a senha correta, e confirma o login **[SS-01a]**
**Então** o acesso é concedido e o servidor é direcionado à sua mesa de trabalho, com as permissões do seu nível/perfil

**Pós-condição:** sessão autenticada no contexto de servidor; sem outra alteração de estado persistente

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-002 Cidadão (Pessoa Física) autentica com sucesso usando CPF e senha corretos**

**Dado** que o cidadão está cadastrado na central de atendimento externo
**Quando** ele acessa a tela de login do cidadão — a mesma usada por empresas (pessoa jurídica), com um único campo de identificação — informa o CPF cadastrado e a senha correta, e confirma o login **[SS-01b]**
**Então** o acesso é concedido ao ambiente virtual do cidadão (acompanhamento de demandas)

**Pós-condição:** sessão autenticada no contexto de cidadão; sem outra alteração de estado persistente

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-003 Empresa (Pessoa Jurídica) autentica com sucesso usando CNPJ e senha corretos**

**Dado** que a empresa está cadastrada como contato externo
**Quando** ela acessa a mesma tela de login do cidadão do CT-002 — campo único, compartilhado com pessoa física — informa o CNPJ cadastrado e a senha correta, e confirma o login **[SS-01b]**
**Então** o acesso é concedido ao ambiente virtual da empresa

**Pós-condição:** sessão autenticada no contexto de empresa; sem outra alteração de estado persistente

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-004 Servidor não consegue autenticar informando um CNPJ no campo de identificação**

**Dado** que a tela de login do servidor está aberta (tela própria, distinta da tela do cidadão)
**Quando** alguém informa um CNPJ (14 dígitos) no campo destinado à identificação do servidor
**E** tenta autenticar
**Então** o sistema não permite o envio ou rejeita a autenticação, porque o servidor deve se autenticar exclusivamente por CPF

**Pós-condição:** nenhuma sessão aberta; nenhuma alteração de estado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-005 Sistema rejeita CPF com dígito verificador inválido em qualquer tela de login**

**Dado** que uma tela de login que exige CPF está aberta (do servidor ou do cidadão)
**Quando** é informado um CPF com 11 dígitos, mas com dígito verificador inválido (ex.: 123.456.789-00)
**E** a pessoa tenta avançar
**Então** o sistema impede o envio ou exibe erro de formato, antes mesmo de consultar as credenciais no cadastro

**Pós-condição:** nenhuma sessão aberta; nenhuma alteração de estado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-006 Sistema rejeita CNPJ com dígito verificador inválido na tela de login do cidadão**

**Dado** que a tela de login do cidadão está aberta (mesma tela compartilhada dos CT-002/CT-003)
**Quando** é informado um CNPJ com dígito verificador inválido (ex.: 12.345.678/0001-00)
**E** a pessoa tenta avançar
**Então** o sistema impede o envio ou exibe erro de formato, antes mesmo de consultar as credenciais no cadastro

**Pós-condição:** nenhuma sessão aberta; nenhuma alteração de estado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-007 Sistema impede um segundo cadastro vinculado a um CPF/CNPJ já existente**

**Dado** que já existe um usuário (servidor, cidadão ou empresa) cadastrado com um determinado CPF/CNPJ
**Quando** alguém inicia um novo cadastro (interno, por link, ou externo)
**E** informa esse mesmo CPF/CNPJ já cadastrado
**E** tenta concluir o cadastro
**Então** o sistema impede a duplicidade e informa que o CPF/CNPJ já está vinculado a um cadastro existente

**Pós-condição:** nenhum novo cadastro é criado; o cadastro original permanece único

> [!note]- Nota
> Este caso cobre um **segundo cadastro explícito** reutilizando o identificador — não é o mesmo cenário do CT-008 (coexistência implícita servidor=cidadão pelo mesmo CPF, que não é um "segundo cadastro", e sim o mesmo indivíduo em dois contextos).

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-008 Servidor autentica na tela do cidadão com o próprio CPF, mas no contexto de cidadão**

**Dado** que o usuário está cadastrado como servidor e, por ser servidor, também possui acesso como cidadão vinculado ao mesmo CPF
**Quando** ele acessa a tela de login do cidadão (não a do servidor) e informa esse mesmo CPF e a senha correta **[SS-01b]**
**Então** o login é bem-sucedido, mas o acesso concedido é o do contexto de cidadão (ambiente de acompanhamento de demandas) — não o da mesa de trabalho de servidor

**Pós-condição:** sessão ativa no contexto de cidadão; nenhuma sessão ou contexto de servidor é aberto a partir desse login

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

## Suite 2 — Validação de credenciais (1.25 / 1.25.2)

### **CT-009 Login recusado quando o CPF/CNPJ está correto mas a senha está incorreta**

**Dado** que o usuário está cadastrado e ativo
**Quando** ele informa um CPF/CNPJ válido e cadastrado, mas uma senha incorreta, e tenta autenticar **[SS-02]**
**Então** o acesso é negado e é exibida uma mensagem de erro clara e genérica, sem indicar se o erro está no identificador ou na senha

**Pós-condição:** nenhuma sessão aberta; a tentativa é contabilizada para efeito de bloqueio (ver Suite 3)

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-010 Login recusado quando o CPF/CNPJ informado não existe na base**

**Dado** que o CPF/CNPJ que será usado não possui cadastro em nenhum tipo de usuário
**Quando** alguém informa esse CPF/CNPJ inexistente com qualquer senha
**E** tenta autenticar
**Então** é exibida uma mensagem de erro clara informando que as credenciais não foram reconhecidas, sem revelar se o CPF/CNPJ existe ou não na base (evitando enumeração de usuários)

**Pós-condição:** nenhuma sessão aberta; nenhuma alteração de estado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-011 Login bloqueado no envio quando CPF/CNPJ e/ou senha estão em branco**

**Dado** que a tela de login está aberta
**Quando** o usuário deixa o campo de CPF/CNPJ e/ou o de senha em branco
**E** tenta autenticar
**Então** o sistema impede o envio e exibe uma mensagem orientando o preenchimento dos campos obrigatórios

**Pós-condição:** nenhuma sessão aberta; nenhuma alteração de estado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-012 As mensagens de erro de login são diferentes para cada motivo de falha**

**Dado** que existem à disposição: um usuário com senha errada, um CPF/CNPJ inexistente, uma conta bloqueada e uma conta inativa
**Quando** o login é tentado para cada um desses quatro cenários (ver CT-009, CT-010, CT-015 e CT-032)
**E** as mensagens exibidas em cada tentativa são comparadas
**Então** cada mensagem é clara e apropriada ao cenário — "conta bloqueada" e "conta inativa" são identificáveis e distintas da mensagem genérica de credenciais não reconhecidas — sem expor dados sensíveis do cadastro

**Pós-condição:** nenhuma sessão aberta; nenhuma alteração de estado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

## Suite 3 — Bloqueio por tentativas (1.25.1)

### **CT-013 Conta é bloqueada exatamente na 5ª tentativa consecutiva de login malsucedida**

**Dado** que o usuário está cadastrado e ativo, sem tentativas malsucedidas recentes
**Quando** são feitas 5 tentativas consecutivas de login com senha incorreta para o mesmo usuário **[SS-03, N=5]**
**Então** a conta é bloqueada a partir da 5ª tentativa, e o sistema exibe uma mensagem informando o bloqueio

**Pós-condição:** conta permanece bloqueada até um desbloqueio (ver CT-016/CT-017)

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-014 Conta não é bloqueada antes de completar 5 tentativas malsucedidas**

**Dado** que o usuário está cadastrado e ativo, sem tentativas malsucedidas recentes
**Quando** são feitas 4 tentativas de login com senha incorreta **[SS-03, N=4]**
**E** na 5ª tentativa é informada a senha correta
**Então** o login é bem-sucedido nessa 5ª tentativa, pois o bloqueio só ocorre ao completar 5 tentativas incorretas seguidas

**Pós-condição:** sessão autenticada normalmente; contador de tentativas malsucedidas zerado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-015 Conta bloqueada continua inacessível mesmo com a senha correta**

**Dado** que a conta já está bloqueada por 5 tentativas malsucedidas anteriores
**Quando** o usuário tenta autenticar informando a senha correta
**Então** o acesso permanece negado, e o sistema exibe uma mensagem informando que a conta está bloqueada

**Pós-condição:** conta permanece bloqueada; nenhuma sessão aberta

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-016 Usuário se desbloqueia sozinho por link recebido em e-mail**

**Dado** que a conta está bloqueada (mesma pré-condição do CT-015)
**Quando** o usuário solicita o desbloqueio, recebe um link por e-mail, e utiliza esse link
**Então** a conta é desbloqueada e o usuário consegue autenticar normalmente em seguida

**Pós-condição:** conta volta ao estado desbloqueado; contador de tentativas malsucedidas zerado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-017 Um servidor desbloqueia manualmente a conta pela tela do servidor**

**Dado** que a conta está bloqueada (mesma pré-condição do CT-015) e existe um servidor com permissão de desbloqueio
**Quando** esse servidor acessa a tela do servidor e realiza o desbloqueio manual da conta
**Então** a conta é desbloqueada e o usuário consegue autenticar normalmente em seguida

**Pós-condição:** conta volta ao estado desbloqueado; contador de tentativas malsucedidas zerado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-018 O contador de tentativas malsucedidas reinicia depois de um login bem-sucedido**

**Dado** que o usuário está cadastrado e ativo
**Quando** são feitas 3 tentativas malsucedidas **[SS-03, N=3]**, depois um login bem-sucedido, e em seguida mais 3 tentativas malsucedidas **[SS-03, N=3]**
**Então** a conta não bloqueia nessa segunda sequência de 3 tentativas, evidenciando que o contador reiniciou após o login bem-sucedido

**Pós-condição:** contador de tentativas malsucedidas em 3 (não bloqueado)

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-019 O bloqueio de um usuário não afeta o acesso de outros usuários**

**Dado** que existem dois usuários distintos, ambos cadastrados e ativos
**Quando** o Usuário A é bloqueado após 5 tentativas malsucedidas **[SS-03, N=5]**
**E** o Usuário B tenta autenticar com suas próprias credenciais corretas
**Então** o Usuário B consegue autenticar normalmente — o bloqueio fica restrito à conta do Usuário A

**Pós-condição:** Usuário A bloqueado; Usuário B com sessão autenticada normalmente

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

## Suite 4 — Ciclo de vida da identidade (1.25.3)

### **CT-020 Servidor "Ativo" tem acesso irrestrito conforme o seu nível de permissão**

**Dado** que o servidor está cadastrado com status "Ativo", em qualquer nível (Administrador, Administrador Setorial, Assistente, Auxiliar ou Visualizador)
**Quando** ele autentica e percorre as funcionalidades disponíveis para o seu nível
**Então** todas as funcionalidades e permissões previstas para o seu perfil estão disponíveis, sem nenhuma restrição adicional ligada ao status funcional

**Pós-condição:** sessão autenticada com permissões plenas do perfil; sem outra alteração de estado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-021 Servidor em "Licença" consegue fazer login, mas com acesso restrito**

**Dado** que o servidor está com status "Licença" vigente
**Quando** ele tenta autenticar normalmente com CPF e senha **[SS-04a, status=Licença]**
**Então** o login é permitido (diferente do que ocorre com "Inativo"), mas o acesso fica restrito a um subconjunto de funcionalidades não transacionais

**Pós-condição:** sessão autenticada em modo restrito (quarentena de privilégios)

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-022 Servidor em "Licença" recebe um aviso de status ao entrar no sistema**

**Dado** que o servidor com status "Licença" vigente já concluiu o login
**Quando** ele observa a tela inicial/mesa de trabalho **[SS-04b, status=Licença]**
**Então** o sistema exibe um aviso/notificação informando que o usuário está em licença

**Pós-condição:** sem alteração de estado além da sessão autenticada em modo restrito

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-023 Servidor em "Licença" não pode modificar nem aprovar nada**

**Dado** que o servidor com status "Licença" está autenticado no sistema
**Quando** ele tenta editar um documento, aprovar/assinar uma tramitação, ou avançar uma etapa de fluxo de trabalho **[SS-04c, status=Licença]**
**Então** todas essas ações de modificação e aprovação ficam bloqueadas/indisponíveis

**Pós-condição:** nenhuma alteração de estado é produzida pelas tentativas bloqueadas

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-024 Servidor em "Licença" não tem acesso a nenhuma funcionalidade, nem de leitura/consulta**

> [!warning]- Correção (18/08)
> Versão anterior deste caso dizia que a leitura/consulta "permanece acessível". Rafael confirmou que isso está **errado**: hoje Licença/Férias não têm visibilidade nenhuma. O Termo (1.25.3.2) fala em "subconjunto mínimo de funcionalidades não transacionais", mas **não exige** que esse mínimo inclua leitura — logo o comportamento real (zero visibilidade) **não é uma inconformidade**.

**Dado** que o servidor com status "Licença" está autenticado no sistema
**Quando** ele tenta consultar/visualizar documentos e processos aos quais teria acesso caso estivesse "Ativo" **[SS-04d, status=Licença]**
**Então** nenhuma dessas funcionalidades de consulta/visualização fica acessível — o acesso é limitado, sem visibilidade de conteúdo

**Pós-condição:** sem alteração de estado além da sessão autenticada em modo restrito

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-025 Mudança de status para "Licença" durante uma sessão ativa é aplicada imediatamente**

**Dado** que o servidor está logado no sistema (sessão ativa) com status "Ativo"
**Quando** um Administrador altera o status dele para "Licença", com início imediato, enquanto a sessão segue aberta
**Então** as restrições de "Licença" passam a valer imediatamente na sessão em curso — não é preciso um novo login para a quarentena de privilégios entrar em vigor

**Pós-condição:** sessão em curso passa a operar em modo restrito a partir do momento da alteração

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-026 Permissões do servidor são restabelecidas automaticamente ao fim da "Licença"**

**Dado** que o servidor tem um período de "Licença" configurado com data de término definida
**Quando** essa data de término chega (ou é simulada) e ele autentica novamente **[SS-05, status=Licença]**
**Então** o sistema restabelece automaticamente as permissões plenas do servidor, que retorna ao status "Ativo"

**Pós-condição:** servidor retorna a status "Ativo", com permissões plenas restauradas

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-027 Servidor em "Férias" consegue fazer login, mas com acesso restrito**

**Dado** que o servidor está com status "Férias" vigente
**Quando** ele tenta autenticar normalmente com CPF e senha **[SS-04a, status=Férias]**
**Então** o login é permitido, com o acesso restrito pela mesma política de quarentena de privilégios aplicada à "Licença"

**Pós-condição:** sessão autenticada em modo restrito (quarentena de privilégios)

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-028 Servidor em "Férias" recebe uma notificação de status ao entrar no sistema**

**Dado** que o servidor com status "Férias" vigente já concluiu o login
**Quando** ele observa a tela inicial/mesa de trabalho **[SS-04b, status=Férias]**
**Então** o sistema exibe uma notificação informando que o usuário está em período de férias

**Pós-condição:** sem alteração de estado além da sessão autenticada em modo restrito

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-029 Servidor em "Férias" tem as permissões de escrita e de fluxo de trabalho suspensas**

**Dado** que o servidor com status "Férias" está autenticado no sistema
**Quando** ele tenta editar/criar um documento, ou executar/avançar uma etapa de fluxo de trabalho **[SS-04c, status=Férias]**
**Então** essas ações de escrita e execução de fluxo ficam bloqueadas/indisponíveis

**Pós-condição:** nenhuma alteração de estado é produzida pelas tentativas bloqueadas

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-030 Servidor em "Férias" não tem acesso a nenhuma funcionalidade, nem de leitura/consulta**

> [!warning]- Correção (18/08)
> Mesma correção do CT-024 — ver nota lá.

**Dado** que o servidor com status "Férias" está autenticado no sistema
**Quando** ele tenta consultar/visualizar documentos e processos aos quais teria acesso caso estivesse "Ativo" **[SS-04d, status=Férias]**
**Então** nenhuma dessas funcionalidades de consulta/visualização fica acessível — o acesso é limitado, sem visibilidade de conteúdo

**Pós-condição:** sem alteração de estado além da sessão autenticada em modo restrito

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-031 Permissões do servidor são restabelecidas automaticamente ao fim das "Férias"**

**Dado** que o servidor tem um período de "Férias" configurado com data de término definida
**Quando** essa data de término chega (ou é simulada) e ele autentica novamente **[SS-05, status=Férias]**
**Então** o sistema restabelece automaticamente as permissões plenas do servidor, que retorna ao status "Ativo"

**Pós-condição:** servidor retorna a status "Ativo", com permissões plenas restauradas

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-032 Servidor "Inativo" nunca consegue autenticar, mesmo com credenciais corretas**

**Dado** que o servidor está cadastrado com status "Inativo" (vínculo encerrado)
**Quando** ele tenta autenticar com CPF e senha corretos
**Então** o acesso é negado de forma explícita (explicit deny), mesmo com credenciais corretas, e o sistema exibe uma mensagem de conta inativa

**Pós-condição:** nenhuma sessão aberta; nenhuma alteração de estado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-033 A mensagem de "conta inativa" é diferente das mensagens de credenciais inválidas e de conta bloqueada**

**Dado** que existem três contas em estados diferentes: uma com CPF/CNPJ inexistente ou senha incorreta, uma bloqueada, e uma inativa
**Quando** o login é tentado nos três cenários
**E** as mensagens exibidas são comparadas
**Então** cada mensagem é claramente identificável, sem ambiguidade entre os três motivos de falha

**Pós-condição:** nenhuma sessão aberta; nenhuma alteração de estado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-034 Mudança de status para "Inativo" durante uma sessão ativa é aplicada imediatamente**

**Dado** que o servidor está logado no sistema (sessão ativa) com status "Ativo"
**Quando** um Administrador altera o status dele para "Inativo" enquanto a sessão segue aberta
**Então** o acesso é revogado imediatamente na sessão em curso — a sessão não continua válida até um novo login

**Pós-condição:** sessão em curso encerrada/revogada a partir do momento da alteração

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-035 Reativar um servidor "Inativo" restabelece o acesso normalmente**

**Dado** que o servidor estava inativo e foi reativado por um Administrador (status volta para "Ativo")
**Quando** ele tenta autenticar novamente
**Então** o login é bem-sucedido, com as permissões plenas do seu perfil totalmente restauradas

**Pós-condição:** servidor retorna a status "Ativo", com permissões plenas restauradas

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-036 Status "Suspenso" se comporta exatamente como "Inativo": negação total de acesso**

**Dado** que o servidor está cadastrado com status "Suspenso"
**Quando** ele tenta autenticar com CPF e senha corretos
**Então** o acesso é negado de forma explícita (explicit deny), do mesmo jeito que o status "Inativo" — nenhum acesso é concedido

**Pós-condição:** nenhuma sessão aberta; nenhuma alteração de estado

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-037 O comportamento de acesso acompanha corretamente cada mudança de status, em sequência**

**Dado** que o servidor está cadastrado e um Administrador pode alterar seu status livremente
**Quando** o status é alternado na sequência Ativo → Férias → Ativo → Licença → Inativo, e após cada mudança é feita uma tentativa de autenticar/agir
**Então** em cada etapa o comportamento de acesso (permitido, restrito ou negado) corresponde exatamente ao status vigente naquele momento

**Pós-condição:** servidor termina a sequência com status "Inativo" e sem acesso

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

## Suite 5 — Transversais e auditoria

### **CT-038 Tentativas de login, com sucesso ou falha, ficam registradas para auditoria**

**Dado** que existe acesso a um relatório/log de auditoria de acessos (perfil Administrador)
**Quando** são feitas tentativas de login com sucesso e com falha para o mesmo usuário
**E** o log/relatório de auditoria é consultado
**Então** todas as tentativas (sucesso e falha) aparecem registradas com data, hora e resultado, permitindo rastreabilidade

**Pós-condição:** log de auditoria com as novas entradas registradas permanentemente

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-039 O sistema permite múltiplas sessões simultâneas do mesmo usuário em dispositivos diferentes**

**Dado** que o usuário está cadastrado e ativo
**Quando** ele autentica em dois dispositivos/navegadores distintos, simultaneamente, com as mesmas credenciais
**Então** ambas as sessões permanecem ativas e funcionais, permitindo requisições normalmente em qualquer uma delas, sem que uma sessão encerre a outra

**Pós-condição:** duas sessões ativas simultâneas para o mesmo usuário

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

## Extra — fora do escopo do Termo de Referência

*Achado durante a revisão: nenhum dos 39 casos acima testa se o **status do servidor** (Licença/Férias/Inativo) afeta o acesso dele **no contexto de cidadão** — já que todo servidor também é cidadão (CT-008). O Termo de Referência não cobre isso, então não vira caso de compliance (não entra na numeração `CT-NNN` nem na reconciliação dos 39). Fica registrado aqui, com shared step próprio, como candidato a suite e2e futura.*

**SS-06** — Verificar se o status não-Ativo do servidor (Licença/Férias/Inativo) afeta o acesso dele no contexto de cidadão, parametrizado por status.

### **CT-E01 [Extra] Servidor em "Licença" — acesso no contexto de cidadão**

**Dado** que o servidor está com status "Licença" e, por ser servidor, também é cidadão pelo mesmo CPF
**Quando** ele acessa a tela de login do cidadão com esse CPF **[SS-06, status=Licença]**
**Então** *[A DEFINIR — fora do escopo do Termo de Referência; candidato a validação e2e]*

**Pós-condição:** *[A DEFINIR]*

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-E02 [Extra] Servidor em "Férias" — acesso no contexto de cidadão**

**Dado** que o servidor está com status "Férias" e, por ser servidor, também é cidadão pelo mesmo CPF
**Quando** ele acessa a tela de login do cidadão com esse CPF **[SS-06, status=Férias]**
**Então** *[A DEFINIR — fora do escopo do Termo de Referência; candidato a validação e2e]*

**Pós-condição:** *[A DEFINIR]*

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

### **CT-E03 [Extra] Servidor "Inativo" — acesso no contexto de cidadão**

**Dado** que o servidor está com status "Inativo" e, por ser servidor, também é cidadão pelo mesmo CPF
**Quando** ele acessa a tela de login do cidadão com esse CPF **[SS-06, status=Inativo]**
**Então** *[A DEFINIR — fora do escopo do Termo de Referência; candidato a validação e2e]*

**Pós-condição:** *[A DEFINIR]*

**Execução Passou?**
- [ ] Sim
- [ ] Não
- [ ] Não se aplica

**Evidências de Testes:**

---

## G. Fora de execução — registro

*Casos considerados e deliberadamente não executados nesta rodada — ficam aqui pra não sumirem do histórico e pra não abrirem buraco na numeração dos ativos.*

| Caso original | Suite | Decisão | Motivo |
|---|---|---|---|
| TC-05 — Empresa não deve conseguir autenticar informando CPF no campo destinado ao CNPJ | Tipos de acesso | **Removido — não se aplica** | A premissa (um campo específico do CNPJ, separado do CPF) não existe: a tela do cidadão tem **campo único**, e a empresa autentica exclusivamente por CNPJ, sem login alternativo por CPF de responsável legal. O cenário como estava escrito não corresponde a nenhuma interação real do sistema. |
