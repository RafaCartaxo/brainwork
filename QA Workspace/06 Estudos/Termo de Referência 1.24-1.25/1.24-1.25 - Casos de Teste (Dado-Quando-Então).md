...
- Organização por suites/shared steps nativos da Qase, com prioridade e rastreabilidade completa por item de requisito: [[QA Workspace/06 Estudos/Termo de Referência 1.24-1.25/1.24-1.25 - Casos organizados para Qase|1.24-1.25 - Casos organizados para Qase.md]] — sincronizada com a mesma numeração desta rodada.

Numeração `CT-001`–`CT-037`, única e contígua ao longo das 5 suites. Três itens da planilha original saíram da numeração ativa — ver seção **G** ao final.
...
> Cidadão CPF e CNPJ usam a **mesma tela**, campo único; servidor tem tela própria; todo servidor também é cidadão (CT-008). Mecanismo de desbloqueio: link por e-mail (CT-015) ou servidor desbloqueando manualmente (CT-016). Mudança de status aplicada imediatamente na sessão em curso (CT-024 Licença, CT-032 Inativo). Licença/Férias hoje não têm acesso de leitura (CT-023/CT-029 — corrigido, o Termo não exige isso). "Suspenso" = "Inativo" (CT-034); múltiplas sessões simultâneas são permitidas (CT-037). Pós-condição em todos os casos ativos.
...
| ID | Ação completa |
|---|---|
| **SS-01a** | Acessar a tela do **servidor** (exclusiva, CPF apenas), informar o CPF cadastrado e a senha correta, e confirmar — usado em CT-001, CT-019 |
| **SS-01b** | Acessar a tela do **cidadão** — campo único de identificação, compartilhado entre CPF (pessoa física) e CNPJ (empresa) — informar o identificador correto e a senha correta, e confirmar — usado em CT-002, CT-003, CT-008 |
| **SS-02** | Acessar a tela de login, informar um identificador válido e uma senha incorreta, e tentar autenticar (1 tentativa) — usado em CT-009 e como base do SS-03 |
| **SS-03** | Repetir o SS-02 **N** vezes seguidas para o mesmo usuário (N varia por caso) — usado em CT-012 (N=5), CT-013 (N=4), CT-017 (N=3, duas vezes), CT-018 (N=5) |
| **SS-04a** | Tentar autenticar normalmente com CPF e senha, estando em afastamento legal — usado em CT-020 (Licença), CT-026 (Férias) |
| **SS-04b** | Já logado, observar a tela inicial/mesa de trabalho — usado em CT-021 (Licença), CT-027 (Férias) |
| **SS-04c** | Tentar uma ação de escrita/aprovação — usado em CT-022 (Licença), CT-028 (Férias) |
| **SS-04d** | Tentar uma ação de leitura/consulta — **hoje sem nenhum acesso, nem de leitura** (o "subconjunto mínimo de funcionalidades não transacionais" do Termo não obriga a incluir visibilidade) — usado em CT-023 (Licença), CT-029 (Férias) |
| **SS-05** | Aguardar (ou simular) a data de término do período de Licença/Férias do servidor, e autenticar novamente — usado em CT-025 (Licença), CT-030 (Férias) |

Os 2 casos de desbloqueio (CT-015/CT-016) não têm shared step próprio — cada um é uma ação única; só reaproveitam a pré-condição "conta bloqueada" do CT-014.
...
> Versão anterior dizia que o sistema barrava o envio antes de checar o cadastro — errado. Não existe validação de formato antes do envio; a requisição segue normal e falha porque não existe conta com aquele CPF (mesmo caminho do CT-010).
...
**Então** o sistema não faz nenhuma validação de formato antes do envio — a requisição segue normal e falha porque não existe nenhuma conta com esse CPF, com a mesma mensagem genérica de credenciais não reconhecidas (mesmo caminho do CT-010)
...
> [!warning]- Correção (19/08)
> Mesma correção do CT-005 — ver nota lá.
...
**E** a pessoa tenta avançar
**Então** o sistema não faz nenhuma validação de formato antes do envio — a requisição segue normal e falha porque não existe nenhuma conta com esse CNPJ, com a mesma mensagem genérica de credenciais não reconhecidas (mesmo caminho do CT-010)
...
> [!note]- Nota
> Este caso cobre um **segundo cadastro explícito** reutilizando o identificador — não é o mesmo cenário do CT-008 (coexistência implícita servidor=cidadão pelo mesmo CPF, que não é um "segundo cadastro").
...
**Então** o acesso é negado e é exibida uma mensagem de erro clara e genérica, sem indicar se o erro está no identificador ou na senha — mensagem diferente da usada quando o identificador não existe, quando a conta está bloqueada, ou quando está inativa (ver CT-010, CT-014, CT-031)
...
**E** tenta autenticar
**Então** é exibida uma mensagem de erro clara informando que as credenciais não foram reconhecidas, sem revelar se o CPF/CNPJ existe ou não na base (evitando enumeração de usuários) — mensagem diferente da usada pra senha incorreta, conta bloqueada, ou conta inativa (ver CT-009, CT-014, CT-031)
...
**Pós-condição:** conta permanece bloqueada até um desbloqueio (ver CT-015/CT-016)
...
**Então** o acesso permanece negado, e o sistema exibe uma mensagem informando que a conta está bloqueada — mensagem diferente das usadas pra credenciais inválidas ou conta inativa (ver CT-009, CT-010, CT-031)
...
**Dado** que a conta está bloqueada (mesma pré-condição do CT-014)
...
**Dado** que a conta está bloqueada (mesma pré-condição do CT-014) e existe um servidor com permissão de desbloqueio
...
### **CT-019 Servidor "Ativo" tem acesso irrestrito conforme o seu nível de permissão**
...
### **CT-020 Servidor em "Licença" consegue fazer login, mas com acesso restrito**
...
### **CT-021 Servidor em "Licença" recebe um aviso de status ao entrar no sistema**
...
### **CT-022 Servidor em "Licença" não pode modificar nem aprovar nada**
...
### **CT-025 Permissões do servidor são restabelecidas automaticamente ao fim da "Licença"**
...
### **CT-026 Servidor em "Férias" consegue fazer login, mas com acesso restrito**
...
### **CT-027 Servidor em "Férias" recebe uma notificação de status ao entrar no sistema**
...
### **CT-028 Servidor em "Férias" tem as permissões de escrita e de fluxo de trabalho suspensas**
...
### **CT-030 Permissões do servidor são restabelecidas automaticamente ao fim das "Férias"**
...
### **CT-031 Servidor "Inativo" nunca consegue autenticar, mesmo com credenciais corretas**
...
**Então** o acesso é negado de forma explícita (explicit deny), mesmo com credenciais corretas, e o sistema exibe uma mensagem de conta inativa — mensagem diferente das usadas pra credenciais inválidas ou conta bloqueada (ver CT-009, CT-010, CT-014)
...
### **CT-033 Reativar um servidor "Inativo" restabelece o acesso normalmente**
...
### **CT-035 O comportamento de acesso acompanha corretamente cada mudança de status, em sequência**
...
### **CT-036 Tentativas de login, com sucesso ou falha, ficam registradas para auditoria**
...
### **CT-037 O sistema permite múltiplas sessões simultâneas do mesmo usuário em dispositivos diferentes**
...
*Achado durante a revisão: nenhum dos casos acima testa se o **status do servidor** (Licença/Férias/Inativo) afeta o acesso dele **no contexto de cidadão** — já que todo servidor também é cidadão (CT-008). O Termo de Referência não cobre isso, então não vira caso de compliance (não entra na numeração `CT-NNN` nem na reconciliação). Fica registrado aqui, com shared step próprio, como candidato a suite e2e futura.*
...
|---|---|---|---|
| TC-05 — Empresa não deve conseguir autenticar informando CPF no campo destinado ao CNPJ | Tipos de acesso | **Removido — não se aplica** | A premissa (um campo específico do CNPJ, separado do CPF) não existe: a tela do cidadão tem **campo único**, e a empresa autentica exclusivamente por CNPJ, sem login alternativo por CPF de responsável legal. |
