...
> [!success] Atualização 31/08 — Fase 2 e Fase 3 (escopo desbloqueado) concluídas
> 18 dos 38 CTs já estão codados (Suíte 1 completa, Suíte 2 completa, Suíte 3 parcial, CT-038) — ver seção "Estado da Fase 2/3" abaixo. **Nenhum desses 18 testes foi executado contra HML ainda** — só escritos e validados por parser (sintaxe), não por execução real. Rodar antes de considerar essa parte encerrada.
...
**Suposições feitas ao codar, pra confirmar depois** (nenhuma delas bloqueia rodar os testes, mas podem exigir ajuste fino do assert depois de rodar contra HML pela primeira vez):
- Status HTTP/corpo exatos de uma tentativa de login falha não são conhecidos (nunca foi capturado um request/response real de falha) — os testes só checam `resp.status` fora de `[200, 201]`, sem checar mensagem de erro específica. Se o backend devolver 200 com erro só no body, os testes vão falhar e vão precisar de ajuste.
