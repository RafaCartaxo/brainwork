...
- [ ] **Reprodução em DEV**: o passo a passo é do ambiente do cliente (PM Guamaré, setor SEMAVM, Ofício 001839/2025). Definir cenário equivalente em DEV — precisa de usuário multi-setor? O MR sugere que o gatilho é o setor ativo do agente (`lastAssociatedSector`) — confirmar se o bug só ocorre com usuário que trocou de setor/switch recente
- [ ] A task não define **critérios de aceite** — derivar do comportamento esperado + regras da doc na hora do destilado
...
Ao criar um novo documento e usar a opção "Associar documentos" na abertura, a busca não retorna documento existente ao qual o servidor tem acesso. O mesmo documento é encontrado normalmente ao associar via despacho. (Reportado pela PM Guamaré; corrigido no MR !537 — busca de documentos associáveis passava consulta sem o setor ativo do agente.)

### Passo a passo para reproduzir

1. Com um servidor posicionado num setor (cenário original: PM Guamaré, setor SEMAVM), iniciar a criação de um novo documento (tipo memorando);
2. Clicar em "Associar documentos" na toolbar inferior do formulário de abertura;
3. No Drawer, buscar por um documento existente ao qual o servidor tem acesso (cenário original: Ofício 001839/2025);
4. Observar que o documento não aparece na listagem.

### Resultado Esperado

O documento ao qual o servidor tem acesso aparece na listagem da busca e pode ser selecionado para associação na abertura.

### Critérios de aceite

- [ ] Busca do Drawer de associação **na abertura** retorna documentos aos quais o servidor tem acesso (paridade com a busca de associação via despacho)
- [ ] Comportamento correto também para usuário multi-setor (setor ativo/último setor associado, com fallback pro setor principal)
- [ ] Sem regressão na associação via despacho
- [ ] Sem regressão na assinatura via toolbar (tela tocada pelo MR)
