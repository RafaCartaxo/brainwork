...
5. **Destilar quando estiver no ponto**: o card nasce em `02 Demandas/<ambiente>/` a partir da seção **Destilado** — só o problema (Descrição objetiva, reprodução, resultado esperado, critérios, CTs), **sem análise nem suposição**; o link pra mesa arquivada vai logo no topo do card (`[!info] Informações`, ou logo após o H1 quando não houver esse callout) — nunca só em Observações, que fica soterrado no fim de cards longos e é fácil de perder quando o card muda de pasta. **Demanda com múltiplos RFs/CAs ou jargão técnico denso** ganha também um `Tasks/<SGV>/<SGV> - Resumo.md` em linguagem simples (sem CT/critério técnico) — não é proporcional pra bug simples. Na daily: `📝 SGV-XXXX - <Tipo> refinado(a) (critérios de aceite prontos)` — **copy oficial do catálogo** ([[../01 Daily/README|01 Daily/README]]) — e, com o card criado, **aplicar a regra de links**: toda menção ao SGV na daily (Atividades, fila, anotações) vira wikilink pro card.
