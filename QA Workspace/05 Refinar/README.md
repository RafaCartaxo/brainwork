...
> [!tip] Atalho: task já vem completa?
> Se o export tem descrição clara, passos, critérios e ambiente definido: **pular a mesa**. Usar [[../../Sistema/Skills/SKILL_LIMPEZA_EXPORT\|SKILL_LIMPEZA_EXPORT]] modo B — limpar e criar card direto em `02 Demandas/` ([[../../Sistema/Skills/SKILL_BUGS\|SKILL_BUGS]] pra bug, [[../../Sistema/Templates/Demanda.md\|Demanda.md]] pra melhoria/funcionalidade). O `05 Refinar/` é pra quando falta informação — não é checkpoint obrigatório.
>
> Pra automação completa: **[[../../Sistema/Agentes/AGENTE_PROCESSAR_EXPORT\|AGENTE_PROCESSAR_EXPORT]]** — dropa o .md e ele decide o caminho sozinho.
...
5. **Destilar quando estiver no ponto**: o card nasce em `02 Demandas/<ambiente>/` a partir da seção **Destilado** — só o problema (Descrição objetiva, reprodução, resultado esperado, critérios, CTs), **sem análise nem suposição**; o link pra mesa arquivada vai logo no topo do card (`[!info] Informações`, ou logo após o H1 quando não houver esse callout) — nunca só em Observações, que fica soterrado no fim de cards longos e é fácil de perder quando o card muda de pasta. **Demanda com múltiplos RFs/CAs ou jargão técnico denso** ganha também um `Tasks/<SGV>/<SGV> - Resumo.md` em linguagem simples (sem CT/critério técnico) — não é proporcional pra bug simples. **Nome de arquivo em `Tasks/<SGV>/` com mais de um item leva prefixo numérico** (`1 - Resumo`, `2 - Refinamento`, `3 - Plano de Automação`; `0 - Índice` no épico) pra ordenar igual à ordem de leitura — sem prefixo, "Refinamento"/"Plano" vêm antes de "Resumo" alfabeticamente, na ordem errada. Detalhe completo em [[../../Sistema/Skills/SKILL_REFINAMENTO|SKILL_REFINAMENTO]]. Na daily: `📝 SGV-XXXX - <Tipo> refinado(a) (critérios de aceite prontos)` — **copy oficial do catálogo** ([[../01 Daily/README|01 Daily/README]]) — e, com o card criado, **aplicar a regra de links**: toda menção ao SGV na daily (Atividades, fila, anotações) vira wikilink pro card.
