...
| Chegou demanda já cadastrada pra refinar (com ou sem análise junto) | 6 | Material bruto → `05 Refinar/` + pendência na fila; card interno com critérios de aceite ([[QA Workspace/Sistema/Skills/SKILL_BUGS#Bug com análise (causa raiz)\|Bug com análise]] quando vier análise) |
...
0. **Material bruto entra em `05 Refinar/`**: export `.md` do Notion, texto colado — do jeito que veio, com pendência `SGV-XXXX - Refinar (material em 05 Refinar/)` na fila. Depois do card criado, o arquivo bruto é apagado ([[QA Workspace/05 Refinar/README|regras da fila]]).

1. **Refinar = criar o card interno**: template certo ([[QA Workspace/Sistema/Templates/Bug Report.md|Bug Report]] pra bug, [[QA Workspace/Sistema/Templates/Demanda.md|Demanda]] pro resto), arquivo já nomeado `<SGV> - <título>`, `task` preenchida, `cadastrado_por` se souber quem cadastrou. O coração do card são os **critérios de aceite** — escritos pra guiar o dev e a revisão de QA depois. Veio um conjunto de material junto (análise de causa raiz, comentários, histórico da task)? Como digerir sem perder informação e onde cada peça entra no card: [[QA Workspace/Sistema/Skills/SKILL_BUGS#Modos de entrada|SKILL_BUGS — Modos de entrada]] e a regra "Bug com análise". **Conferir também o Status/sprint da task na plataforma** — ele diz em que ponto da esteira a demanda já está e calibra a pendência do passo 4 (ex.: "aguardando dev" vs "MR em revisão" mudam completamente o próximo passo; anotar o status visto em Observações do card).
2. **Registrar na daily**: `📝 SGV-XXXX - <Tipo> refinado(a) (critérios de aceite prontos)` em Atividades — ex.: `Bug refinado`, `Melhoria refinada`.
3. **Levar pro Notion**: registrar a análise/critérios na task da plataforma e marcar na daily: `📤 SGV-XXXX - <Tipo> atualizado(a) no Notion (análise/critérios registrados na task)`. O ciclo de definição fecha aqui: ideia → a refinar → refinada → atualizada na plataforma.
4. **Pendências automáticas — todo refinamento gera DUAS**, na hora, em **A fazer hoje**:
   - `SGV-XXXX - Atualizar no Notion (levar análise/critérios pra task)` — a materialização do passo 3; ao concluir, vira o `📤` (inclui os pontos "definir com dev/produto" que os critérios levantarem);
   - o **próximo passo da esteira**, calibrado pelo Status visto no passo 1 — ex.: `SGV-XXXX - Revisar cenários de teste quando o dev entregar` (aguardando dev) ou `SGV-XXXX - Revisar cenários de teste (MR em revisão)` (dev já entregou).
